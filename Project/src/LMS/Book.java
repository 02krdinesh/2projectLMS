...
public class Book {
    ...
    {
        if (!HoldRequestOperations.holdRequests.isEmpty())
        { 
            System.out.println("\nHold Requests are: ");
            ...
            for (int i = 0; i < HoldRequestOperations.holdRequests.size(); i++)
            {                      
                System.out.print(i + "-" + "\t\t");
                HoldRequestOperations.holdRequests.get(i).print();
            }
        }
        else
            System.out.println("\nNo Hold Requests.");                                
    ...
    {
        return HoldRequestOperations.holdRequests;
    }
    /*-----------------------------------*/
   ...
   // Request for Holding a Book
    public void makeHoldRequest(Borrower borrower)
    {
        ...
        for (int i = 0; i < HoldRequestOperations.holdRequests.size(); i++)
        {
            if ((HoldRequestOperations.holdRequests.get(i).getBorrower() == borrower))
            {
                makeRequest = false;    
                break;
            }
    ...
    {        
        ...
        ArrayList<HoldRequest> hRequests = HoldRequestOperations.holdRequests;
        ...
        {               
            if (!HoldRequestOperations.holdRequests.isEmpty())
            {
                ...
                for (int i = 0; i < HoldRequestOperations.holdRequests.size() && !hasRequest;i++)
                {
                    if (HoldRequestOperations.holdRequests.get(i).getBorrower() == borrower)
                        hasRequest = true;
                ...
                if (hasRequest)
                {
                    ...
                    if (HoldRequestOperations.holdRequests.get(0).getBorrower() == borrower)
                        serviceHoldRequest(HoldRequestOperations.holdRequests.get(0));
