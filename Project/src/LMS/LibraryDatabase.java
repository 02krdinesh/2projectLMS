...
/**
 * Moved database / JDBC responsibilities here.
 * Methods are similar to the original implementation but accept a Library instance.
 */
public class LibraryDatabase {
    ...
    public LibraryDatabase(String jdbcUrl, String user, String pass) {
        this.host = jdbcUrl;
        this.user = user;
        this.pass = pass;
    }
    ...
    public void populateLibrary(Library lib, Connection con) throws SQLException, IOException
    {       
        if (lib == null || con == null) return;
            ...
            do
                {
                    ...
                    if(lib.getLibrarian()!=null && iid==lib.getLibrarian().getID())
                    {
                    ...
                    {
                        if(lib.getLibrarian()!=null && rd==lib.getLibrarian().getID())
            ...
            do
                {
                    ...
                    ArrayList<Entity> persons = lib.getPersons();
                    ...
                    for(int i=0;i<books.size() && set;i++)
                    {
                        if(books.get(i).getID()==bokid)
                        {
                          ...
                          lib.holdRequestsOperations.addHoldRequest(hbook);
            ...
            do
                {
                    ...
                    ArrayList<Loan> books = lib.loans;
        ...
        Entity.setIDCount(max);  
    }
    
    
    // Filling Changes back to Database
    public void fillItBack(Library lib, Connection con) throws SQLException,SQLIntegrityConstraintViolationException
    {
        if (lib == null || con == null) return;
        ...
        String[] clearStatements = {
            ...
            "DELETE FROM LIBRARY.BORROWED_BOOK",
            "DELETE FROM LIBRARY.ON_HOLD_BOOK",
            "DELETE FROM LIBRARY.BOOK",
            "DELETE FROM LIBRARY.CLERK",
            "DELETE FROM LIBRARY.LIBRARIAN",
            "DELETE FROM LIBRARY.BORROWER",
            "DELETE FROM LIBRARY.STAFF",
            "DELETE FROM LIBRARY.PERSON"
        };

        for (String tmpl : clearStatements) {
            try (PreparedStatement p = con.prepareStatement(tmpl)) {
                p.executeUpdate();
            }
        }
        ...
        {
            String template = "INSERT INTO LIBRARY.PERSON (ID,PNAME,PASSWORD,ADDRESS,PHONE_NO) values (?,?,?,?,?)";
            try (PreparedStatement stmt = con.prepareStatement(template)) {
        ...
        for(int i=0;i<lib.getPersons().size();i++)
        {
            if (lib.getPersons().get(i).getClass().getSimpleName().equals("Borrower"))
            {
                String template = "INSERT INTO LIBRARY.BORROWER(B_ID) values (?)";
                try (PreparedStatement stmt = con.prepareStatement(template)) {
                    stmt.setInt(1, lib.getPersons().get(i).getID());
                    stmt.executeUpdate();
                }
        ...
        {
            String template = "INSERT INTO LIBRARY.BOOK (ID,TITLE,AUTHOR,SUBJECT,IS_ISSUED) values (?,?,?,?,?)";
            try (PreparedStatement stmt = con.prepareStatement(template)) {
        ...
        for(int i=0;i<lib.loans.size();i++)
        {
            String template = "INSERT INTO LIBRARY.LOAN(L_ID,BORROWER,BOOK,ISSUER,ISS_DATE,RECEIVER,RET_DATE,FINE_PAID) values (?,?,?,?,?,?,?,?)";
            try (PreparedStatement stmt = con.prepareStatement(template)) {
                stmt.setInt(1,i+1);
                stmt.setInt(2,lib.loans.get(i).getBorrower().getID());
                stmt.setInt(3,lib.loans.get(i).getBook().getID());
                stmt.setInt(4,lib.loans.get(i).getIssuer().getID());
                stmt.setTimestamp(5,new java.sql.Timestamp(lib.loans.get(i).getIssuedDate().getTime()));
                stmt.setBoolean(8,lib.loans.get(i).getFineStatus());
                if(lib.loans.get(i).getReceiver()==null)
                {
                ...
                {
                    stmt.setInt(6,lib.loans.get(i).getReceiver().getID());  
                    stmt.setTimestamp(7,new java.sql.Timestamp(lib.loans.get(i).getReturnDate().getTime()));
                }
        ...
        for(int i=0;i<lib.getBooks().size();i++)
        {
            ...
            {
                String template = "INSERT INTO LIBRARY.ON_HOLD_BOOK(REQ_ID,BOOK,BORROWER,REQ_DATE) values (?,?,?,?)";
                try (PreparedStatement stmt = con.prepareStatement(template)) {
                    stmt.setInt(1,x);
        ...
        for(int i=0;i<lib.getBooks().size();i++)
        {
            ...
            {
                boolean set=true;
                for(int j=0;j<lib.loans.size() && set ;j++)
                {
                    if(lib.getBooks().get(i).getID()==lib.loans.get(j).getBook().getID())
                    {
                        if(lib.loans.get(j).getReceiver()==null)
                        {
                            String template = "INSERT INTO LIBRARY.BORROWED_BOOK(BOOK,BORROWER) values (?,?)";
                            try (PreparedStatement stmt = con.prepareStatement(template)) {
                                stmt.setInt(1,lib.loans.get(j).getBook().getID());
                                stmt.setInt(2,lib.loans.get(j).getBorrower().getID());
                                stmt.executeUpdate();
                            }
                            set=false;
                        }
